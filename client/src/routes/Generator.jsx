    ...
    try {
      ...
      if (data.success) {
        ...
        setMessages(prev => [...prev, { 
          role: 'ai', 
          content: 'I have generated the website based on your requirements. You can view and edit the files in the editor.' 
        }]);
      } else {
          ...
          {messages.map((message, index) => (
            <div key={index} className={`flex items-start gap-4 ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`flex-1 ${message.role === 'user' ? 'text-right' : ''}`}>
                <div className={`inline-block rounded-lg px-4 py-2 text-sm max-w-[85%]
                  ${message.role === 'ai' ? 'bg-gray-800 text-gray-300' : 'bg-indigo-500/10 text-indigo-400'}`}>
                  {message.content}
                </div>
              </div>
            </div>
